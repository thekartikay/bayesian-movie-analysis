...
priors = read.csv('dataset/movie-dataset-priors.csv')

PRIORS_MODE = c()
for (i in 1:dim(priors)[2]){
  PRIORS_MODE = append(PRIORS_MODE, Mode(priors[,i]))
}

#PRIOR_MEAN = c(0.0001, 0.55, 0.85, 0.145, 0.97, 0.12, 0.18, 0.12)
PRIORS_STD = c()
for (i in 1:dim(priors)[2]){
  PRIORS_STD = append(PRIORS_STD, sd(priors[,i]))
}
#PRIOR_STD = c(0.004,0.17,0.16,0.12,0.04,0.09,0.11,0.08)
SEED1 = 12
SEED2 = 3
SEED3 = 7
# Let's find some priors for us to build out model with. For the prior,
# we will take a small sample of out dataset and use them to  generate priors
...
genMCMC = function( data , xName="x" , yName="y" , 
  ...
  dataList = list(
    x = x ,
    y = y ,
    prior_mode = PRIORS_MODE,
    prior_std = PRIORS_STD,
    Nx = dim(x)[2] ,
    Px = length(PRIORS_MODE),
    Ntotal = dim(x)[1]
  )
  #-----------------------------------------------------------------------------
  ...
  runJagsOut <- run.jags( method=runjagsMethod ,
                          model="models/logistic-model.txt" , 
                          monitor=parameters , 
                          data=dataList ,  
...
myData = read.csv('dataset/movie-dataset-with-scores.csv')
train = myData[myData[,"release_year"] <= 2010,]
test = myData[myData[,"release_year"] > 2010,]
write.csv(train,'dataset/train.csv')
write.csv(test,'dataset/test.csv')

xName = c('runtime', 'actor_name_score', 'studio_score','crew_member_score', 
          'season_autumn', 'season_spring','season_summer')
yName = "target"
fileNameRoot = "movie-success-prediction-"
...
write.csv(as.matrix(mcmcCoda),'dataset/model.csv', row.names = FALSE)
plotMCMC( mcmcCoda , test , xName=xName , yName=yName,
                     showCurve=TRUE ,  pairsPlot=TRUE ,
                     saveName='results/dec-1-test-' , saveType="jpg" )